...
public final class OneloAuthModule {
    ...
    // MARK: - Convenience proxies

    /// Current restored session, or `nil` if anonymous / not yet restored.
    /// Forwarded from `authObject.currentSession`.
    public var currentSession: OneloSession? { authObject.currentSession }

    /// `true` once Onelo Auth has finished restoring the session from Keychain.
    /// Events emitted before this becomes `true` will ship with
    /// `user_id = "anonymous"` even for signed-in users — see `awaitReady`.
    public var isReady: Bool { authObject.isReady }

    /// Suspend until session restoration finishes (i.e. `isReady` becomes
    /// `true`), then return. Use before emitting monitor events at startup
    /// that need accurate `user_id`. Throws `OneloError.timeout` after the
    /// given timeout (default 5s, well above typical restore latency).
    ///
    /// ```swift
